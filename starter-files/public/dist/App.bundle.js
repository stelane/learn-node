...
function autocomplete(input, latInput, lngInput) {
  if (!input) return; // skip from running if there is no input on page
...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    ...
    if (isHearted) {
      ...
      setTimeout(function () {
        return _this.heart.classList.remote('heart__button--float');
      }, 2500);
    }
  }).catch(console.error);
}
...
function searchResultsHTML(stores) {
  // take in array of stores and return html
  return stores.map(function (store) {
    return '\n      <a href="/store/' + store.slug + '" class="search__result">\n        <strong>' + store.name + '</strong>\n      </a>\n    ';
  }).join(''); // we're getting an array back, want to make it a string.
}
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    // if they arn't pressing arrow up, down or enter, then skip it
